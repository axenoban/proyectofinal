    ...
    <table class="table table-striped datatable" style="width:100%">
      <thead>
        <tr>
          ...
          <th>Categoría</th>
          <th>Estado</th>
          <th>Tiempo Prep. (min)</th>
          <th>Acciones</th>
        </tr>
      </thead>
      <tbody>
        <?php if (count($platos) === 0): ?>
          <tr><td colspan="7" class="text-center">No hay platos registrados.</td></tr>
        <?php else: ?>
          <?php foreach ($platos as $plato): ?>
            <tr>
              ...
              <td>
                ...
                <a href="/proyectofinal/controllers/PlatoController.php?action=delete&id=<?= $plato['id'] ?>" class="btn btn-sm btn-danger" title="Eliminar" onclick="return confirm('¿Seguro que quieres eliminar este plato?');">
